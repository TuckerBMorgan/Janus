...
class SLAM(object):
  ...
  def process_frame(self, img, pose=None, verts=None):
    ...
    if len(self.mapp.points) > 0:
      print(len(self.mapp.points))
      # project the last 10 the map points into the current frame
...
if __name__ == "__main__":
  ...
  # camera parameters
  W = 216#int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
  H = 384#int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

  CNT = 1000#int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
  F = 525#float(os.getenv("F", "525"))
  ...
  i = 0
  image_file_name = 1
  while image_file_name < 350:
#    ret, frame = cap.read()
    ret = True
    frame = cv2.imread("Images/" + str(image_file_name) + ".png")
    image_file_name += 1
